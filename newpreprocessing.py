...
class FakeNewsPreprocessor:
    def __init__(self, n_process=4):
        ...
        # Domain is specifically excluded from NLP processing to keep it as is, but we still want to clean it and keep it as a feature
    ...
    def clean_domain(self, series):
        """Fast safe cleaning for domain column"""
        return (
            series.fillna("")
            .astype(str)
            .str.lower()
    ...
    def process_chunk(self, chunk):
        """Near C-speed processing (no shuffle)"""

        # Clean text columns and track which ones are active for processing
        active_cols = []
        ...
        # Clean domain separately if it exists
        ...
        # Flatten text columns for better speed
        texts = []
        col_lengths = []
        ...
        # Tokenization, stopword removal, stemming in one pass with spaCy (parallelized) so its faster
        ...
        # Update vocab counters after processing the batch
        ...
        # Rebuilding the columns in the chunk with processed texts
        idx = 0
    ...
    def load_and_process(self, input_csv, output_csv, chunksize=30_000, sample_frac=0.2):
        ...
        # include domain in CSV reading, but NOT NLP processing
    ...
    def report_vocab_statistics(self):
        ...
        print(f"Reduction: {stopword_reduction:.2%}")
        print(f"After stemming: {vocab_after_stem_size}")
        print(f"Reduction: {stemming_reduction:.2%}")
...
if __name__ == "__main__":
    ...
    processor.load_and_process(
        ...
        chunksize=30_000,
        sample_frac=0.2 # Set to 1.0 for full processing, 0.2 for 20% sampling or whatever number you want
    )